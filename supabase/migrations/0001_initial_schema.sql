...
create table lane_assignments (
  ...
  tournament_team_id bigint references tournament_teams(id) on delete cascade,  -- NULL=미편성 그룹
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table lane_assignment_players (
  ...
  tournament_player_id bigint not null references tournament_players(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (lane_assignment_id, tournament_player_id)
);
...
create table game_states (
  ...
  unique (tournament_event_id, game_number)
);
...
begin
  ...
  -- 마감된 마지막 게임 번호 (없으면 0)
  select coalesce(max(game_number), 0) into v_max_locked_game
  from game_states
  where tournament_event_id = p_event_id and status = 'locked';
  ...
  left join scores s
   ...
   and s.game_number <= v_max_locked_game
  group by tp.id
...
-- 게임 마감 RPC (트랜잭션 단위)
create or replace function lock_game(p_event_id bigint, p_game_number smallint)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  insert into game_states (tournament_event_id, game_number, status, locked_at, locked_by)
  values (p_event_id, p_game_number, 'locked', now(), auth.uid())
  on conflict (tournament_event_id, game_number)
...
-- 게임 마감 해제 (super_admin만)
create or replace function unlock_game(p_event_id bigint, p_game_number smallint)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  where tournament_event_id = p_event_id and game_number = p_game_number;
